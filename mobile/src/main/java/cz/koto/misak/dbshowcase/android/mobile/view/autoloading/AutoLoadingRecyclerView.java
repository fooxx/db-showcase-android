...
import cz.koto.misak.dbshowcase.android.mobile.entity.autoloading.ILoading;
import cz.koto.misak.dbshowcase.android.mobile.entity.autoloading.OffsetAndLimit;
import cz.koto.misak.dbshowcase.android.mobile.util.BackgroundExecutor;
import cz.koto.misak.dbshowcase.android.mobile.view.StatefulLayout;
...
public class AutoLoadingRecyclerView<T, U extends RecyclerView.ViewHolder> extends RecyclerView {
    ...
    protected boolean mFirstPortionLoaded;
    protected boolean mAllPortionsLoaded;
    public boolean mUseMultiThreading = true;
    public DataLoadType mCurrentLoadingType;
    private LoadViewDataListener loadViewDataListener = null;
    ...
    public void startReLoading() {
        ...
        mFirstPortionLoaded = false;
        mAllPortionsLoaded = false;
        startLoading();
    }

    public void startLoading() {
        startLoading(DataLoadType.NONE);
    }
     ...
     */
    public void startLoading(DataLoadType dataLoadType) {
        mCurrentLoadingType = dataLoadType;
        // if all data was loaded then new download is not needed
        if (mAllPortionsLoaded) {
            return;
        }
        // if first portion was loaded then subscribe to LoadingChannel
        if (mFirstPortionLoaded) {
    ...
    protected void subscribeToLoadingChannel() {
        Subscriber<OffsetAndLimit> toLoadingChannelSubscriber = new Subscriber<OffsetAndLimit>() {
            ...
            public void onError(Throwable e) {
                ...
                if ((loadViewDataListener != null) && (mCurrentLoadingType == DataLoadType.ONLINE)) {
                    loadViewDataListener.doLoadingOffline();
                } else {
                    setLayoutOffline();
                }
            }
    ...
    protected void loadNewItems(OffsetAndLimit offsetAndLimit) {
        Subscriber<List<T>> loadNewItemsSubscriber = new Subscriber<List<T>>() {
            ...
            public void onError(Throwable e) {
                ...
                if ((loadViewDataListener != null) && (mCurrentLoadingType == DataLoadType.ONLINE)) {
                    loadViewDataListener.doLoadingOffline();//offline fallback
                } else {
                    setLayoutOffline();
                    subscribeToLoadingChannel();
                }
        ...
        Observable<List<T>> observableList = getLoadingObservable().getLoadingObservable(offsetAndLimit);
        if (mUseMultiThreading) {
            observableList.subscribeOn(Schedulers.from(BackgroundExecutor.getSafeBackgroundExecutor()));
        }
        observableList.observeOn(AndroidSchedulers.mainThread());
        loadNewItemsSubscription = observableList.subscribe(loadNewItemsSubscriber);
    }
    ...
    public Parcelable onSaveInstanceState() {
        ...
        ss.firstPortionLoadedSaved = mFirstPortionLoaded;
        ss.allPortionsLoadedSaved = mAllPortionsLoaded;
        return ss;
    }
    ...
    public void onRestoreInstanceState(Parcelable state) {
        ...
        mFirstPortionLoaded = ss.firstPortionLoadedSaved;
        mAllPortionsLoaded = ss.allPortionsLoadedSaved;
    }
    ...
    /**
     * Disable subscribers to work on parallel thread.
     */
    public void disableMultiThreading() {
        this.mUseMultiThreading = false;
    }

    /**
     * Enable subscribers on parallel thread.
     * Default option.
     */
    ...
    public LoadViewDataListener getLoadViewDataListener() {
        return loadViewDataListener;
    }

    public void setLoadViewDataListener(LoadViewDataListener loadViewDataListener) {
        this.loadViewDataListener = loadViewDataListener;
    }
}
